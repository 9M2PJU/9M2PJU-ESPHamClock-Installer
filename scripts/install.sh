...
echo -e "\n${YELLOW}[1/4] Checking and installing dependencies...${NC}"

if [ "$OS" = "Linux" ]; then
    if has_cmd apt-get; then
        echo "Detected Debian/Ubuntu/Raspberry Pi OS system."
        sudo apt-get update -y
        sudo apt-get install -y build-essential make g++ libx11-dev libgpiod-dev curl unzip pkg-config
    elif has_cmd pacman; then
        echo "Detected Arch Linux / CachyOS / Manjaro system."
        sudo pacman -Sy --noconfirm base-devel libx11 libgpiod curl unzip
    elif has_cmd dnf; then
        echo "Detected Fedora / RHEL system."
        sudo dnf install -y gcc-c++ make libX11-devel libgpiod-devel curl unzip
    else
        echo "Please ensure g++, make, and libX11-dev are installed."
    fi
elif [ "$OS" = "Darwin" ]; then
    echo "Detected macOS."
...
if [ "$OS" = "Linux" ]; then
    ...
    # Check deploy/ or root for icon
    if [ -f "$SCRIPT_DIR/deploy/hamclock.png" ]; then
        cp "$SCRIPT_DIR/deploy/hamclock.png" "$HOME/.local/share/icons/"
    elif [ -f "$SCRIPT_DIR/hamclock.png" ]; then
        cp "$SCRIPT_DIR/hamclock.png" "$HOME/.local/share/icons/"
    fi
    # Check deploy/ or root for desktop shortcut
    if [ -f "$SCRIPT_DIR/deploy/hamclock.desktop" ]; then
        cp "$SCRIPT_DIR/deploy/hamclock.desktop" "$HOME/.local/share/applications/"
        sed -i "s|/usr/local/bin/hamclock|$BIN_DEST/hamclock|g" "$HOME/.local/share/applications/hamclock.desktop" || true
    elif [ -f "$SCRIPT_DIR/hamclock.desktop" ]; then
        cp "$SCRIPT_DIR/hamclock.desktop" "$HOME/.local/share/applications/"
        sed -i "s|/usr/local/bin/hamclock|$BIN_DEST/hamclock|g" "$HOME/.local/share/applications/hamclock.desktop" || true
    fi
fi
